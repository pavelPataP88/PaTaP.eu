...
Официальные принципы, изученные до кода:

| Принцип | Оригинальное решение PaTaP | Почему это безопасно |
|---|---|---|
| Microsoft Teams Walkie Talkie: PTT удерживается во время речи и отпускается после речи; активный канал и говорящий явно видны. | Одна крупная кнопка «Зажми и говори»; отдельный явный активный канал; состояния «готово / запись / отправка / говорит другой водитель / ошибка». | Водитель понимает, когда микрофон реально работает и в какой канал уйдёт передача; нет фоновой записи или автозапуска. |
| Android permissions: чувствительный доступ нужно запрашивать как можно позже, после конкретного действия пользователя, и постоянно показывать доступ к микрофону. | getUserMedia остаётся только после осознанного удержания PTT; во время записи кнопка и статус получают постоянное состояние RECORDING. | Микрофон не запускается при открытии экрана и не работает скрытно. |
| W3C Pointer Events: низкоуровневый pointer UX должен иметь эквивалентную клавиатурную обработку. | Сохраняются Space/Enter hold-to-talk; Escape отменяет текущую передачу; pointercancel/потеря активного удержания не должны случайно отправлять запись. | Пользователь может отменить действие, а assistive/keyboard input получает эквивалентный путь. |
| WCAG Pointer Cancellation: действие должно быть отменяемым до завершения, если down-event начинает взаимодействие. | Короткое случайное нажатие не отправляется; явная отмена прекращает запись и освобождает pending lease через существующий DELETE с upload token. | Снижает случайные голосовые отправки и не ослабляет серверную модель доступа. |

Официальные источники:
- https://support.microsoft.com/en-US/teams/platform/communicate-with-your-team-in-walkie-talkie
- https://support.microsoft.com/en-US/teams/apps-service/get-started-with-teams-walkie-talkie
- https://developer.android.com/guide/topics/permissions/overview
- https://developer.android.com/media/platform/mediarecorder
- https://www.w3.org/TR/pointerevents4/
- https://www.w3.org/TR/WCAG22/

Техническое решение до кода:
- серверный протокол, `server/radio/routes.js`, `server/radio/repository.js`, SQLite и upload token/lease не менять;
- улучшить только Driver radio UX и добавить клиентские regression-тесты;
- при сетевой неопределённости не показывать ложное «отправлено»: успех только после подтверждённого upload response; при неясном результате сообщать, что доставка не подтверждена;
- короткую запись считать случайным нажатием и отменять через уже существующий cancel endpoint;
- не добавлять фоновой записи, геолокации, новых каналов или внешних сервисов.
