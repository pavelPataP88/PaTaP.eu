...
export function createDriverModule(context) {
  ...
  const gpsLabel = { ACTIVE: "В сети", STALE: "Позиция устарела", OFF: "GPS выключен" };
  ...
  function render(value) {
    ...
    for (const [label, value] of [["Тип", driver.driverType], ["Транспорт", driver.vehicle], ["Страна", countryLabel(driver.countryCode)], ["GPS", driver.gps], ["Обновлено", driver.locationUpdatedAt ? new Date(driver.locationUpdatedAt).toLocaleString() : "нет позиции"]]) {
      if (!value) continue;
    ...
    contactButton.textContent = driver.relationship === "CONTACT" ? "В контактах" : driver.relationship === "REQUEST_SENT" ? "Запрос отправлен" : driver.relationship === "REQUEST_INCOMING" ? "Принять запрос" : "В контакты";
    contactButton.disabled = ["CONTACT", "REQUEST_SENT", "BLOCKED"].includes(driver.relationship);
  ...
  document.querySelector("#driver-card-contact").addEventListener("click", (event) => {
    if (!driver || ["CONTACT", "BLOCKED"].includes(driver.relationship)) return;
