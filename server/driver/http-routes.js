const { DRIVER_RADII_KM, validLocation } = require("./location");
const { normalizeDriverProfile, publicDriverProfile } = require("./profile");
const { createDriverRuntime } = require("./runtime");
const {
  CONFIRMATIONS,
...
function createDriverRoutes(options) {
  const {
    db,
    json,
    requireSession,
  ...
  const runtime = options.runtime || createDriverRuntime({
    db,
    json,
    requireSession,
    requireCsrf,
  ...
  const handleDriverRoute = async function handleDriverRoute(req, res, url, body) {
  ...
  handleDriverRoute.start = runtime.start;
  handleDriverRoute.stop = runtime.stop;
  handleDriverRoute.runtimeState = runtime.state;
  return handleDriverRoute;
}
