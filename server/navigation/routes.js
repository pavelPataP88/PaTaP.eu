...
const {createNominatimGeocoder}=require("./geocoders/nominatim");
const {createNavigationService}=require("./service");

function createNavigationRoutes(options,{roadReports=null,provider=null,geocoder=null}={}){
  const navigation=createNavigationService({db:options.db,provider:provider||createValhallaProvider(),geocoder:geocoder||createNominatimGeocoder(),roadReports,nowIso:options.nowIso});
  ...
  function requireRead(req,res,key,limit=60,minutes=1){const session=requireUser(req,res);if(!session)return null;if(!options.checkRate(`navigation:${key}:user:${session.user.id}`,limit,minutes)){respond(res,429,{error:"navigation_rate_limited"});return null;}return session;}
  ...
  return async function handleNavigationRoute(req,res,url,body){
    ...
    if(req.method==="GET"&&url.pathname==="/api/driver/navigation/search"){
      const session=requireRead(req,res,"search",80,1);if(!session)return true;const q=String(url.searchParams.get("q")||"");if(q.trim().length<2)return respond(res,200,{query:q.trim(),results:[],partial:false,warnings:[]});
      const nearLat=Number(url.searchParams.get("lat")),nearLon=Number(url.searchParams.get("lon")),near=Number.isFinite(nearLat)&&Number.isFinite(nearLon)?{latitude:nearLat,longitude:nearLon}:null;
      try{return respond(res,200,await navigation.search(session.user.id,q,{limit:Number(url.searchParams.get("limit"))||8,near}));}catch(error){return serviceError(res,error);}
    }
