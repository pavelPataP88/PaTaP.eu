...
const PLACE_STATUSES = new Set(["ACTIVE","COMMUNITY_UNVERIFIED","CLOSED","REMOVED"]);
const PLACE_KINDS = new Set(["TRUCK_PARKING","REST_AREA","SERVICE_AREA","MOP","SECURE_PARKING","YARD","OTHER"]);
const FEE_MODES = new Set(["FREE","PAID","MIXED","UNKNOWN"]);

const INPUT_TO_COLUMN = Object.freeze({
  status:"status", kind:"kind", name:"name", latitude:"latitude", longitude:"longitude", countryCode:"country_code", country_code:"country_code",
  address:"address", road:"road", direction:"direction", operator:"operator", phone:"phone", website:"website", openingHours:"opening_hours", opening_hours:"opening_hours",
  capacityTruck:"capacity_truck", capacity_truck:"capacity_truck", capacityTotal:"capacity_total", capacity_total:"capacity_total",
  feeMode:"fee_mode", fee_mode:"fee_mode", priceText:"price_text", price_text:"price_text", currency:"currency",
  maxLengthM:"max_length_m", max_length_m:"max_length_m", maxHeightM:"max_height_m", max_height_m:"max_height_m", maxWeightT:"max_weight_t", max_weight_t:"max_weight_t",
  certificationLevel:"certification_level", certification_level:"certification_level", certificationSource:"certification_source", certification_source:"certification_source",
  certificationValidUntil:"certification_valid_until", certification_valid_until:"certification_valid_until",
  bookingProvider:"booking_provider", booking_provider:"booking_provider", bookingUrl:"booking_url", booking_url:"booking_url", bookingPhone:"booking_phone", booking_phone:"booking_phone",
  bookingInstructions:"booking_instructions", booking_instructions:"booking_instructions", dataConfidence:"data_confidence", data_confidence:"data_confidence"
});
for (const column of BOOL_FIELDS) {
  const camel = column.replace(/_([a-z])/g, (_, c) => c.toUpperCase());
  INPUT_TO_COLUMN[camel] = column;
  INPUT_TO_COLUMN[column] = column;
}
...
function bool(value) { return value === true || value === 1 || value === "1" || ["yes","true","designated","permissive"].includes(String(value || "").toLowerCase()); }
...
function canonicalKey(input) {
  ...
  return `${country(input.countryCode || input.country_code) || "XX"}:${lat}:${lon}:${key}`;
}
function hasOwn(input, ...keys) { return keys.some((key) => Object.prototype.hasOwnProperty.call(input || {}, key)); }
function meaningfulString(value) { return value !== null && value !== undefined && text(value).length > 0; }
...
function createParkingRepository(db, { nowIso = () => new Date().toISOString() } = {}) {
  ...
  function normalizePlace(input, { community = false } = {}) {
    ...
    const rawKind = String(input.kind || "TRUCK_PARKING").toUpperCase();
    const rawStatus = community ? "COMMUNITY_UNVERIFIED" : String(input.status || "ACTIVE").toUpperCase();
    const rawFee = String(input.feeMode ?? input.fee_mode ?? "UNKNOWN").toUpperCase();
    const rawCert = community ? "NONE" : String(input.certificationLevel ?? input.certification_level ?? "NONE").toUpperCase();
    const out = {
      canonical_key: canonicalKey({ ...input, latitude, longitude, name }),
      status: PLACE_STATUSES.has(rawStatus) ? rawStatus : "ACTIVE",
      kind: PLACE_KINDS.has(rawKind) ? rawKind : "TRUCK_PARKING",
      name, latitude, longitude,
      country_code: country(input.countryCode ?? input.country_code),
      address: text(input.address,300), road: text(input.road,80), direction: text(input.direction,80), operator: text(input.operator,120),
      phone: text(input.phone,80), website: text(input.website,400), opening_hours: text(input.openingHours ?? input.opening_hours,200),
      capacity_truck: Number.isSafeInteger(Number(input.capacityTruck ?? input.capacity_truck)) && Number(input.capacityTruck ?? input.capacity_truck) >= 0 ? Number(input.capacityTruck ?? input.capacity_truck) : null,
      capacity_total: Number.isSafeInteger(Number(input.capacityTotal ?? input.capacity_total)) && Number(input.capacityTotal ?? input.capacity_total) >= 0 ? Number(input.capacityTotal ?? input.capacity_total) : null,
      fee_mode: FEE_MODES.has(rawFee) ? rawFee : "UNKNOWN",
      price_text: text(input.priceText ?? input.price_text,160), currency: text(input.currency,3).toUpperCase(),
      max_length_m: num(input.maxLengthM ?? input.max_length_m), max_height_m: num(input.maxHeightM ?? input.max_height_m), max_weight_t: num(input.maxWeightT ?? input.max_weight_t),
      certification_level: Object.hasOwn(CERT_ORDER, rawCert) ? rawCert : "NONE",
      certification_source: community ? "" : text(input.certificationSource ?? input.certification_source,160),
      certification_valid_until: community ? null : (input.certificationValidUntil ?? input.certification_valid_until ?? null),
      booking_provider: community ? "" : text(input.bookingProvider ?? input.booking_provider,100),
      booking_url: community ? "" : text(input.bookingUrl ?? input.booking_url,500),
      booking_phone: community ? "" : text(input.bookingPhone ?? input.booking_phone,80),
      booking_instructions: community ? "" : text(input.bookingInstructions ?? input.booking_instructions,500),
      data_confidence: clamp(num(input.dataConfidence ?? input.data_confidence) ?? (community ? 0.4 : 0.65), 0, 1)
    };
    for (const field of BOOL_FIELDS) {
      const camel = field.replace(/_([a-z])/g, (_, c) => c.toUpperCase());
      out[field] = community && field === "reservable" ? 0 : (bool(input[camel] ?? input[field]) ? 1 : 0);
    }
    return out;
  }

  function sparseImportPatch(input, normalized) {
    const patch = {};
    for (const [key, column] of Object.entries(INPUT_TO_COLUMN)) {
      if (!Object.prototype.hasOwnProperty.call(input || {}, key)) continue;
      if (Object.prototype.hasOwnProperty.call(normalized, column)) patch[column] = normalized[column];
    }
    patch.name = normalized.name;
    patch.latitude = normalized.latitude;
    patch.longitude = normalized.longitude;
    if (hasOwn(input,"countryCode","country_code")) patch.country_code = normalized.country_code;
    return patch;
  }

  function mergeImportedRow(existing, normalized, input, authority, topAuthority, now, verifiedAt) {
    const patch = sparseImportPatch(input, normalized);
    const higherOrEqual = authority >= topAuthority;
    const next = {};
    const identityFields = new Set(["name","latitude","longitude","country_code","kind","status"]);
    const numericFields = new Set(["capacity_truck","capacity_total","max_length_m","max_height_m","max_weight_t"]);
    const protectedAuthorityFields = new Set(["certification_level","certification_source","certification_valid_until","booking_provider","booking_url","booking_phone","booking_instructions"]);

    for (const [column, value] of Object.entries(patch)) {
      if (column === "data_confidence") continue;
      const current = existing[column];
      if (BOOL_FIELDS.includes(column)) {
        // Source adapters often cannot distinguish "not mentioned" from false. Never let an absent/false field erase a confirmed true.
        if (Number(value) === 1 && (higherOrEqual || Number(current) !== 1)) next[column] = 1;
        continue;
      }
      if (identityFields.has(column)) {
        if (higherOrEqual && value !== null && value !== undefined && (typeof value !== "string" || meaningfulString(value))) next[column] = value;
        continue;
      }
      if (protectedAuthorityFields.has(column)) {
        const meaningful = value !== null && value !== undefined && value !== "" && value !== "NONE";
        if (higherOrEqual && meaningful) next[column] = value;
        continue;
      }
      if (numericFields.has(column)) {
        if (value !== null && value !== undefined && (higherOrEqual || current === null || current === undefined)) next[column] = value;
        continue;
      }
      if (column === "fee_mode") {
        if (value !== "UNKNOWN" && (higherOrEqual || current === "UNKNOWN")) next[column] = value;
        continue;
      }
      const meaningful = value !== null && value !== undefined && (typeof value !== "string" || meaningfulString(value));
      if (meaningful && (higherOrEqual || current === null || current === undefined || current === "")) next[column] = value;
    }
    next.updated_at = now;
    if (verifiedAt) next.last_verified_at = verifiedAt;
    if (!Object.keys(next).length) return;
    const columns = Object.keys(next);
    db.prepare(`UPDATE parking_places SET ${columns.map((c)=>`${c}=?`).join(",")} WHERE id=?`).run(...columns.map((c)=>next[c]), existing.id);
  }

  function ensurePreferences(userId, now = nowIso()) {
    db.prepare(`INSERT OR IGNORE INTO parking_user_preferences(user_id,vehicle_class,max_detour_km,updated_at) VALUES(?, 'TIR', 10, ?)`).run(userId,now);
  ...
  function updatePreferences(userId,input,now=nowIso()) {
    ...
    const dimension=(key,currentValue)=>input[key]===undefined?currentValue:(input[key]===null||input[key]===""?null:num(input[key]));
    const lengthM=dimension("lengthM",current.length_m),heightM=dimension("heightM",current.height_m),weightT=dimension("weightT",current.weight_t);
  ...
  function reviewSummary(placeId) {
    ...
    const out=(v)=>v===null?null:Number(Number(v).toFixed(2));
    return {count:Number(row.n||0),overall:out(row.overall),security:out(row.security),cleanliness:out(row.cleanliness),access:out(row.access_rating),quietness:out(row.quietness)};
  }

  function prediction(placeId,now=nowIso()) {
    const target=new Date(now),weekday=target.getUTCDay(),hour=target.getUTCHours();
    const rows=db.prepare("SELECT status,observed_at FROM parking_occupancy_observations WHERE place_id=? AND observed_at>=? ORDER BY observed_at DESC LIMIT 500")
      .all(placeId,new Date(Date.parse(now)-180*24*3600_000).toISOString());
    const samples=rows.filter((r)=>{const d=new Date(r.observed_at);const diff=Math.abs(d.getUTCHours()-hour);const hourDiff=Math.min(24-diff,diff);return d.getUTCDay()===weekday&&hourDiff<=2&&Object.hasOwn(OCCUPANCY_SCORE,r.status);});
  ...
  function liveOccupancy(placeId, now=nowIso()) {
    ...
    const rows=db.prepare(`SELECT * FROM parking_occupancy_observations WHERE place_id=? AND observed_at>=? ORDER BY observed_at DESC LIMIT 100`)
      .all(placeId,new Date(nowTime-7*24*3600_000).toISOString());
    const active=rows.filter((r)=>{const age=(nowTime-nowMs(r.observed_at))/60000;if(r.expires_at&&Date.parse(r.expires_at)<=nowTime)return false;return ["OFFICIAL","OPERATOR"].includes(r.source_type)?age<=30:age<=120;});
    ...
    if(drivers.length){
      const latestByUser=new Map();for(const r of drivers){const key=String(r.user_id||r.source_key||r.id);if(!latestByUser.has(key))latestByUser.set(key,r);}
      const unique=[...latestByUser.values()];let totalWeight=0,score=0;const counts={AVAILABLE:0,LIMITED:0,FULL:0,CLOSED:0,UNKNOWN:0};
      for(const r of unique){const age=Math.max(0,(nowTime-nowMs(r.observed_at))/60000);const weight=Math.exp(-age/65);totalWeight+=weight;score+=(OCCUPANCY_SCORE[r.status]??0.5)*weight;counts[r.status]=(counts[r.status]||0)+1;}
      const avg=score/Math.max(totalWeight,0.001);const status=avg>=1.05?"CLOSED":avg>=0.82?"FULL":avg>=0.45?"LIMITED":"AVAILABLE";const agreement=Math.max(...Object.values(counts))/unique.length;const confidence=clamp(0.45+Math.min(unique.length,5)*0.08+agreement*0.15,0,0.92);const latest=unique.reduce((a,b)=>nowMs(a.observed_at)>nowMs(b.observed_at)?a:b);
  ...
  function publicPlace(row,userId,{origin=null,prefs=null,includeSources=false}={}) {
    if(!row)return null;const preferences=prefs||ensurePreferences(userId);const occupancy=liveOccupancy(Number(row.id));const reviews=reviewSummary(Number(row.id));const fit=vehicleFit(row,preferences);const favorite=Boolean(db.prepare("SELECT 1 FROM parking_favorites WHERE user_id=? AND place_id=?").get(userId,row.id));const distanceKm=origin?haversineKm(origin.latitude,origin.longitude,row.latitude,row.longitude):null;const securityScore=Number(row.restricted_access)+Number(row.cctv)+Number(row.guard)+Number(row.fence)+Number(row.gate)+Number(row.lighting)+Number(row.personal_access_control)+CERT_ORDER[row.certification_level]*2;
    ...
    if(includeSources)result.sources=sourceSummary(Number(row.id));return result;
  }
  function parseFilters(input={}) {
    const amenities=String(input.amenities||"").split(",").map((v)=>v.trim()).filter((v)=>AMENITY_QUERY_FIELDS.has(v));const security=String(input.security||"").split(",").map((v)=>v.trim()).filter((v)=>SECURITY_QUERY_FIELDS.has(v));const certification=String(input.certification||"NONE").toUpperCase();
  ...
  function filterAndRank(rows,userId,{origin=null,filters={},limit=50,routeDistance=null}={}) {
    ...
    for(const row of rows){if(parsed.countryCode&&row.country_code!==parsed.countryCode)continue;if(parsed.kind&&row.kind!==parsed.kind)continue;if(parsed.query&&!`${row.name} ${row.operator} ${row.address} ${row.road}`.toLocaleLowerCase("und").includes(parsed.query))continue;if(parsed.amenities.some((k)=>!row[k]))continue;if(parsed.security.some((k)=>!row[k]))continue;if(CERT_ORDER[row.certification_level]<CERT_ORDER[parsed.certification])continue;if(parsed.reservable&&!row.reservable)continue;const card=publicPlace(row,userId,{origin,prefs});if(parsed.availableOnly&&!["AVAILABLE","LIMITED"].includes(card.occupancy.status))continue;const routeKm=routeDistance?routeDistance(row):null;let rank=0;rank+=card.fit.compatible?0:1000;rank+=(100-card.fit.score)*1.8;rank+=card.occupancy.status==="CLOSED"?700:card.occupancy.status==="FULL"?450:card.occupancy.status==="LIMITED"?80:card.occupancy.status==="UNKNOWN"?35:0;rank+=(card.distanceKm??routeKm??0)*8;if(routeKm!==null)rank+=routeKm*15;if(prefs.secure_only)rank-=Math.min(card.security.score,12)*6;rank-=Math.min(card.reviews.overall||0,5)*8;rank-=card.dataConfidence*30;if(card.favorite)rank-=25;if(card.booking.reservable)rank-=4;out.push({...card,routeDistanceKm:routeKm===null?null:Number(routeKm.toFixed(3)),_rank:rank});}
    out.sort((a,b)=>a._rank-b._rank||((a.distanceKm??99999)-(b.distanceKm??99999))||a.id-b.id);return out.slice(0,Math.min(Math.max(Number(limit)||50,1),100)).map(({_rank,...item})=>item);
  }
  ...
  function placeDetails(userId,placeId,{origin=null}={}) {
    const row=placeRow(placeId);if(!row)return null;const place=publicPlace(row,userId,{origin,includeSources:true});const reviews=db.prepare(`SELECT r.*,p.nickname FROM parking_reviews r JOIN driver_profiles p ON p.user_id=r.user_id WHERE r.place_id=? ORDER BY r.updated_at DESC LIMIT 100`).all(placeId).map((r)=>({author:r.nickname,overall:Number(r.overall),security:r.security,cleanliness:r.cleanliness,access:r.access_rating,quietness:r.quietness,text:r.text,visitedAt:r.visited_at,createdAt:r.created_at,updatedAt:r.updated_at,isMine:Number(r.user_id)===Number(userId)}));const photos=db.prepare(`SELECT ph.id,ph.mime_type,ph.byte_length,ph.file_name,ph.created_at,p.nickname FROM parking_photos ph LEFT JOIN driver_profiles p ON p.user_id=ph.uploader_id WHERE ph.place_id=? AND ph.state='VISIBLE' ORDER BY ph.created_at DESC LIMIT 40`).all(placeId).map((r)=>({id:Number(r.id),mimeType:r.mime_type,byteLength:Number(r.byte_length),fileName:r.file_name,createdAt:r.created_at,uploader:r.nickname||null,url:`/api/driver/parking/photos/${r.id}/content`}));const alternatives=search(userId,{latitude:row.latitude,longitude:row.longitude,radiusKm:50,limit:8}).filter((p)=>p.id!==Number(placeId)).slice(0,3);return {place,reviews,photos,alternatives};
  }
  ...
  function reportOccupancy(userId,placeId,input,now=nowIso()) {
    if(!placeRow(placeId))return {error:"parking_not_found",status:404};const status=String(input?.status||"").toUpperCase();if(!Object.hasOwn(OCCUPANCY_SCORE,status))return {error:"invalid_parking_occupancy",status:400};const free=input.freeSpots===undefined||input.freeSpots===null?null:Number(input.freeSpots),total=input.totalSpots===undefined||input.totalSpots===null?null:Number(input.totalSpots);if((free!==null&&(!Number.isSafeInteger(free)||free<0))||(total!==null&&(!Number.isSafeInteger(total)||total<0))||(free!==null&&total!==null&&free>total))return {error:"invalid_parking_occupancy",status:400};db.prepare(`INSERT INTO parking_occupancy_observations(place_id,source_type,source_key,user_id,status,free_spots,total_spots,note,observed_at,expires_at,created_at) VALUES(?,'DRIVER','',?,?,?,?,?,?,?,?)`).run(placeId,userId,status,free,total,text(input.note,160),now,addMinutes(now,120),now);return {occupancy:liveOccupancy(placeId,now)};
  }
  function addOfficialOccupancy(placeId,input,now=nowIso()) {
    const status=String(input.status||"UNKNOWN").toUpperCase();if(!Object.hasOwn(OCCUPANCY_SCORE,status)||!placeRow(placeId))return false;const sourceType=["OFFICIAL","OPERATOR","IMPORT"].includes(input.sourceType)?input.sourceType:"OFFICIAL";db.prepare(`INSERT INTO parking_occupancy_observations(place_id,source_type,source_key,user_id,status,free_spots,total_spots,note,observed_at,expires_at,created_at) VALUES(?,?,?,NULL,?,?,?,?,?,?,?)`).run(placeId,sourceType,text(input.sourceKey,120),status,input.freeSpots??null,input.totalSpots??null,text(input.note,160),input.observedAt||now,input.expiresAt||addMinutes(input.observedAt||now,30),now);return true;
  }
  function upsertReview(userId,placeId,input,now=nowIso()) {
    if(!placeRow(placeId))return {error:"parking_not_found",status:404};const rating=(v,required=false)=>v===undefined||v===null?(required?NaN:null):Number(v);const overall=rating(input.overall,true),security=rating(input.security),cleanliness=rating(input.cleanliness),access=rating(input.access),quietness=rating(input.quietness);if(![overall,security,cleanliness,access,quietness].every((v)=>v===null||(Number.isInteger(v)&&v>=1&&v<=5)))return {error:"invalid_parking_review",status:400};const existing=db.prepare("SELECT created_at FROM parking_reviews WHERE place_id=? AND user_id=?").get(placeId,userId);db.prepare(`INSERT INTO parking_reviews(place_id,user_id,overall,security,cleanliness,access_rating,quietness,text,visited_at,created_at,updated_at) VALUES(?,?,?,?,?,?,?,?,?,?,?) ON CONFLICT(place_id,user_id) DO UPDATE SET overall=excluded.overall,security=excluded.security,cleanliness=excluded.cleanliness,access_rating=excluded.access_rating,quietness=excluded.quietness,text=excluded.text,visited_at=excluded.visited_at,updated_at=excluded.updated_at`).run(placeId,userId,overall,security,cleanliness,access,quietness,text(input.text,1200),input.visitedAt||null,existing?.created_at||now,now);return {reviews:reviewSummary(placeId)};
  }
  ...
  function createCommunityPlace(userId,input,now=nowIso()) {
    const normalized=normalizePlace(input,{community:true});if(!normalized)return {error:"invalid_parking_place",status:400};const nearby=db.prepare("SELECT * FROM parking_places WHERE status!='REMOVED' AND latitude BETWEEN ? AND ? AND longitude BETWEEN ? AND ?").all(normalized.latitude-0.002,normalized.latitude+0.002,normalized.longitude-0.003,normalized.longitude+0.003).find((r)=>haversineKm(normalized.latitude,normalized.longitude,r.latitude,r.longitude)<=0.15&&normalizedKey(r.name)===normalizedKey(normalized.name));if(nearby)return {error:"parking_possible_duplicate",status:409,existingId:Number(nearby.id)};const cols=Object.keys(normalized),values=Object.values(normalized);const result=db.prepare(`INSERT INTO parking_places(${cols.join(",")},created_by,created_at,updated_at,last_verified_at) VALUES(${cols.map(()=>"?").join(",")},?,?,?,?)`).run(...values,userId,now,now,null);const placeId=Number(result.lastInsertRowid);const raw=JSON.stringify(input);db.prepare(`INSERT INTO parking_place_sources(place_id,source_type,external_id,authority,source_name,source_url,licence_text,source_updated_at,imported_at,raw_json,raw_hash) VALUES(?,'PATAP_COMMUNITY',?,?,?,?,?,?,?,?,?)`).run(placeId,`driver:${userId}:${placeId}`,SOURCE_AUTHORITY.PATAP_COMMUNITY,"PaTaP Driver","","PaTaP community contribution",now,now,raw,crypto.createHash("sha256").update(raw).digest("hex"));return {place:publicPlace(placeRow(placeId),userId,{includeSources:true}),created:true};
  }
  ...
  function upsertImportedPlace(input,source,now=nowIso()) {
    const normalized=normalizePlace(input);if(!normalized)return {error:"invalid_import_record"};const sourceType=String(source.type||"OTHER").toUpperCase();if(!Object.hasOwn(SOURCE_AUTHORITY,sourceType)&&sourceType!=="OTHER")return {error:"invalid_import_source"};const externalId=text(source.externalId,180);if(!externalId)return {error:"invalid_import_source"};const authority=clamp(Number(source.authority??SOURCE_AUTHORITY[sourceType]??45),0,100);
    ...
    if(!row){const candidates=db.prepare("SELECT * FROM parking_places WHERE status!='REMOVED' AND latitude BETWEEN ? AND ? AND longitude BETWEEN ? AND ?").all(normalized.latitude-0.003,normalized.latitude+0.003,normalized.longitude-0.004,normalized.longitude+0.004);row=candidates.find((r)=>haversineKm(normalized.latitude,normalized.longitude,r.latitude,r.longitude)<=0.18&&(normalizedKey(r.name)===normalizedKey(normalized.name)||(normalized.operator&&normalizedKey(r.operator)===normalizedKey(normalized.operator))))||null;}
    if(!row){const cols=Object.keys(normalized);const result=db.prepare(`INSERT INTO parking_places(${cols.join(",")},created_at,updated_at,last_verified_at) VALUES(${cols.map(()=>"?").join(",")},?,?,?)`).run(...Object.values(normalized),now,now,source.sourceUpdatedAt||now);row=placeRow(Number(result.lastInsertRowid));created=true;}else{const top=Number(db.prepare("SELECT COALESCE(MAX(authority),-1) a FROM parking_place_sources WHERE place_id=?").get(row.id).a);mergeImportedRow(row,normalized,input,authority,top,now,source.sourceUpdatedAt||now);row=placeRow(row.id);}
    const raw=typeof source.raw==="string"?source.raw:JSON.stringify(source.raw??input);const hash=crypto.createHash("sha256").update(raw).digest("hex");db.prepare(`INSERT INTO parking_place_sources(place_id,source_type,external_id,authority,source_name,source_url,licence_text,source_updated_at,imported_at,raw_json,raw_hash) VALUES(?,?,?,?,?,?,?,?,?,?,?) ON CONFLICT(source_type,external_id) DO UPDATE SET place_id=excluded.place_id,authority=excluded.authority,source_name=excluded.source_name,source_url=excluded.source_url,licence_text=excluded.licence_text,source_updated_at=excluded.source_updated_at,imported_at=excluded.imported_at,raw_json=excluded.raw_json,raw_hash=excluded.raw_hash`).run(row.id,sourceType,externalId,authority,text(source.name,160),text(source.url,500),text(source.licence,300),source.sourceUpdatedAt||null,now,raw,hash);
    const authorities=db.prepare("SELECT authority FROM parking_place_sources WHERE place_id=?").all(row.id).map((r)=>Number(r.authority));const confidence=clamp((Math.max(...authorities,40)/100)+(authorities.length>1?0.05:0),0,0.99);db.prepare("UPDATE parking_places SET data_confidence=?,updated_at=? WHERE id=?").run(confidence,now,row.id);return {placeId:Number(row.id),created};
  }
