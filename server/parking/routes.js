...
function createParkingRoutes(options){
  ...
  return async function handleParkingRoute(req,res,url,body){
    ...
    if(req.method==="POST"&&photoUpload&&body===undefined){const session=requireMutation(req,res,"photo",12,10);if(!session)return true;const type=mime(req.headers["content-type"]);if(!type)return respond(res,415,{error:"unsupported_parking_photo"});let binary;try{binary=await readBinary(req,MAX_PHOTO_BYTES);}catch(error){return respond(res,error.status||400,{error:error.message||"invalid_parking_photo"});}if(!binary.length)return respond(res,400,{error:"empty_parking_photo"});if(!validImageBytes(type,binary))return respond(res,415,{error:"invalid_parking_photo_signature"});const ext=type==="image/png"?"png":type==="image/webp"?"webp":"jpg";const storageKey=`${crypto.randomUUID()}.${ext}`;fs.mkdirSync(storageDir,{recursive:true,mode:0o700});const finalPath=path.join(storageDir,storageKey),temp=`${finalPath}.${crypto.randomUUID()}.tmp`;try{fs.writeFileSync(temp,binary,{flag:"wx",mode:0o600});fs.renameSync(temp,finalPath);const value=parking.registerPhoto(session.user.id,Number(photoUpload[1]),{storageKey,mimeType:type,byteLength:binary.length,fileName:safeName(req.headers["x-file-name"])},options.nowIso());if(value.error){fs.rmSync(finalPath,{force:true});return result(res,value);}options.audit(req,"parking_photo_added",{userId:session.user.id,success:true,details:{placeId:Number(photoUpload[1]),photoId:value.photo.id}});return respond(res,201,value);}catch{fs.rmSync(temp,{force:true});fs.rmSync(finalPath,{force:true});return respond(res,500,{error:"parking_photo_upload_failed"});}}
