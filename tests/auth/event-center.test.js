...
test("Event Center creates URGENT nearby road events and exposes SSE readiness",async()=>{
  ...
  const controller=new AbortController();const streamPromise=fetch(`${baseUrl}/api/driver/events/stream`,{headers:nearby.client.headers({Accept:"text/event-stream"}),signal:controller.signal});const response=await streamPromise;assert.equal(response.status,200);assert.match(response.headers.get("content-type")||"",/text\/event-stream/);const reader=response.body.getReader();const first=await reader.read();assert.match(Buffer.from(first.value||[]).toString("utf8"),/event\.ready/);controller.abort();
