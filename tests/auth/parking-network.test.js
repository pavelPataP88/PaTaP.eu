...
test("Parking Network supports canonical place search vehicle fit favorites reviews corrections and trust boundaries",async()=>{
  const driver=await createDriver("core");await setLocation(driver,50.2649,19.0238);
  const place=await createPlace(driver,"PaTaP Test Parking",50.2660,19.0250,{maxHeightM:4.1,adrAllowed:false,frigoPower:true,certificationLevel:"PLATINUM",certificationSource:"driver-self-claim",reservable:true,bookingProvider:"Fake",bookingUrl:"https://fake.example"});
  assert.equal(place.status,"COMMUNITY_UNVERIFIED");assert.equal(place.security.certificationLevel,"NONE");assert.equal(place.booking.reservable,false);assert.equal(place.booking.url,"");
  ...
  r=await driver.client.request("/api/driver/parking/search?lat=50.2649&lon=19.0238&radiusKm=10&amenities=shower&security=cctv");assert.equal(r.response.status,200);const found=r.data.places.find(p=>p.id===place.id);assert.ok(found);assert.equal(found.fit.compatible,false);assert.ok(found.fit.issues.includes("height_limit"));assert.ok(found.fit.issues.includes("adr_not_supported"));assert.equal(found.security.certificationLevel,"NONE");assert.equal(found.booking.reservable,false);
