...
function nextTestNickname(suffix) {
  registrationSequence += 1;
  const safeSuffix = String(suffix).replace(/[^a-zA-Z0-9_-]/g, "").slice(0, 8) || "driver";
  return `Road_${safeSuffix}_${runNicknameTag}_${registrationSequence.toString(36)}`;
}
...
async function register(client, suffix, withProfile = true) {
  ...
  if (withProfile) {
    result = await client.request("/api/driver/profile", { method: "PUT", body: { nickname: nextTestNickname(suffix), driverType: "TIR", countryCode: "PL" } });
    assert.equal(result.response.status, 201);
  }
}
