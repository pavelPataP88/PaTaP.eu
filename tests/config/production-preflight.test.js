...
function fixtureDb(file) {
  ...
  db.exec(`
    ...
    INSERT INTO radio_transmissions(id,state,expires_at,byte_length) VALUES
      (1,'COMMITTED','2026-08-22T06:00:00.000Z',1200),
      (2,'COMMITTED','2026-08-22T08:00:00.000Z',3400),
      (3,'PENDING','2026-08-22T05:00:00.000Z',5000);
    ...
    INSERT INTO road_reports(id,expires_at,closed_at) VALUES
      (1,'2026-08-22T08:00:00.000Z',NULL),
      (2,'2026-08-22T06:00:00.000Z',NULL),
      (3,'2026-08-22T08:00:00.000Z','2026-08-22T06:30:00.000Z');
  `);
  db.close();
}
...
test("verified backup is an independent integrity-checked copy and does not mutate source", async (t) => {
  ...
  const file = path.join(dir, "auth.sqlite");
  fixtureDb(file);
  const backupDir = path.join(dir, "preflight");

  const info = await createVerifiedBackup(file, backupDir, "fixture");
...
test("live road snapshot preserves the old in-memory payload and blocks restart while reports are active", async (t) => {
  ...
  const fetchImpl = async (url) => {
    ...
    return {
      ...
      status: 200,
      async json() {
        return { reports: [{ id: 7, type: "ACCIDENT", lane: "LEFT", latitude: 50, longitude: 19, createdAt: "2026-08-22T06:30:00.000Z", expiresAt: "2026-08-22T07:30:00.000Z", confirmations: { active: 1, gone: 0 } }] };
      }
    };
  };

  const snapshot = await captureRoadReportsSnapshot({
  ...
  const decision = evaluatePreflight({
    database: { integrity: { ok: true }, authMigrations: { present: true, contiguous: true, supported: true }, push: { activeSubscriptions: 0 }, radioRetention: { expiredCommitted: 0 } },
    backupInfo: { integrity: { ok: true } },
    roadSnapshot: snapshot,
...
test("active push subscriptions require preserved valid VAPID material but secret keys never enter the report", async (t) => {
  ...
  const blocked = evaluatePreflight({
    database: { integrity: { ok: true }, authMigrations: { present: true, contiguous: true, supported: true }, push: { activeSubscriptions: 2 }, radioRetention: { expiredCommitted: 0 } },
    backupInfo: { integrity: { ok: true } },
    roadSnapshot: { status: "PASS", activeCount: 0 },
    vapid: { exists: false, valid: false }
  });
  ...
  const ready = evaluatePreflight({
    database: { integrity: { ok: true }, authMigrations: { present: true, contiguous: true, supported: true }, push: { activeSubscriptions: 2 }, radioRetention: { expiredCommitted: 4 } },
    backupInfo: { integrity: { ok: true } },
    roadSnapshot: { status: "PASS", activeCount: 0 },
    vapid
  });
