...
const {ensureNavigationSchema}=require("../../server/navigation/schema.js");
...
test("Navigation keeps one active route per user and removes old inactive route history",()=>{
  ...
  insert("route-000000000003","2026-03-05T00:00:00.000Z");rows=db.prepare("SELECT id,status FROM navigation_routes ORDER BY id").all();assert.deepEqual(rows,[{id:"route-000000000003",status:"ACTIVE"}]);db.close();
});
